...
def gen_serendipidade(c):
    ...
    graph = {}
    c.execute("select count(*)*100/(select count(*) from user_recommendations _ur, users _u where _u.id = _ur.target_id and _u.stage_number >=6), 'amigos' from ratings r, user_recommendations ur, users u where r.product_id = ur.product_id and r.user_id = ur.target_id and u.id = r.user_id and u.stage_number >=6 and r.stars >= 4 and r.unknown='t' and ur.sender_id in (select _u.id from users _u where _u.group_id = u.group_id and _u.id <> u.id) union all select count(*)*100/(select count(*) from user_recommendations _ur, users _u where _u.id = _ur.target_id and _u.stage_number >=6), 'desconhecidos' from ratings r, user_recommendations ur, users u where r.product_id = ur.product_id and r.user_id = ur.target_id and u.id = r.user_id and u.stage_number >=6 and r.stars >= 4 and r.unknown='t' and ur.sender_id in (select _u.id from users _u where _u.group_id <> u.group_id and _u.id <> u.id)")
    for taxa, algoritmo in c.fetchall():
        graph[algoritmo] = [taxa]
